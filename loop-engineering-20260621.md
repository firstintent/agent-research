...
> **来源**：https://addyo.substack.com/p/loop-engineering
> **作者**：Addy Osmani · **发表**：2026-06-08 · **出处**：Elevate (Substack)
> **抓取日期**：2026-06-21

> 副标题：From prompting agents to designing the systems that prompt them.
...
I believe this *may* be the future of how we work with coding agents. However, its still early, I'm skeptical and you absolutely *have* to be careful about **token costs** (usage patterns can vary wildly if you are token rich or poor), so I want to unpack what it is and what it means.
...
Both products have all five now. The names are a bit different here and there but the capability is the same thing. Let me go one by one because honestly the details are where a loop either holds together or quietly leaks everywhere.
...
## 五大支柱速查（Loop Engineering 组件 → Claude Code / Codex 映射）

| 组件 | 作用 | Claude Code | Codex |
|---|---|---|---|
| Automations（心跳） | 定时发现/triage，自动化触发 | `/loop`（按周期重跑）、cron、hooks、GitHub Actions | Automations 标签页 + Triage 收件箱 |
| `/goal`（收敛循环） | 跑到可验证停止条件为止，独立小模型判完成 | `/goal` | `/goal`（含 pause/resume/clear） |
| Worktrees | 并行 agent 文件隔离 | `git worktree`、`--worktree`、subagent `isolation: worktree` | 内建 worktree 支持 |
| Skills | 写下项目知识，免每次重述 | `.claude/` SKILL.md | `$`/`/skills`，SKILL.md |
| Plugins/Connectors | 接入真实工具（MCP） | MCP | MCP |
| Sub-agents（造者/检者分离） | 一个写、另一个查 | `.claude/agents/` + agent teams | `.codex/agents/` TOML |
| Memory（第六件） | 会话外持久状态 | 磁盘 markdown / Linear board | 同 |

**核心警示**：① token 成本随用法差异极大；② 验证仍在人；③ comprehension debt 会更快累积；④ 警惕 cognitive surrender——"build it like someone who intends to stay the engineer, not just the person who presses go."
